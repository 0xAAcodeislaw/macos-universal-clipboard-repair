...
- 通用剪贴板单向失效：Mac 可以复制到 iPhone，但 iPhone 无法复制到 Mac，或者方向相反
...
先从最简单、风险最低的操作开始。多数临时故障不需要退出 iCloud、重置网络或修改钥匙串。

### 第一步：分别重启两端设备

1. 先重启无法正常“发送”剪贴板内容的设备，开机并解锁后等待约 30 秒，再用苹果“备忘录”复制一小段纯文字测试。
2. 如果仍未恢复，再重启负责“接收”和粘贴内容的另一台设备，然后重新测试。
3. 如果无法判断是哪一端异常，直接重启 iPhone / iPad 和 Mac 两端即可。

单向故障可以帮助判断问题所在：

- Mac → iPhone 正常，但 iPhone → Mac 失败：优先重启 iPhone。
- iPhone → Mac 正常，但 Mac → iPhone 失败：优先重启 Mac；如果重启后仍未恢复，再使用本 App 的 Handoff / Universal Clipboard 修复。

本项目已经实际记录过一次单向故障：Mac → iPhone 正常，iPhone → Mac 失败；Mac 能发现 iPhone，但手机没有发布共享剪贴板标志，最终在重启 iPhone 后恢复。这类故障发生在手机端，Mac 上反复重启 `useractivityd`、`sharingd` 和 `pboard` 不会解决问题。

### 第二步：检查基本条件

- 两端登录同一个 Apple 账户。
- 两端的 Wi-Fi、蓝牙和 Handoff（接力）均已开启。
- 两台设备保持解锁并放在附近，先用少量纯文字测试。
- 如果使用的是测试版系统，优先安装可用的最新系统更新。

### 第三步：修复 Mac 端服务

如果重启后仍是 Mac 端无法发送或接收，再运行本 App 的 Handoff / Universal Clipboard 修复。该按钮只能重启 Mac 上的相关服务，无法直接修复或重启 iPhone / iPad 内部的剪贴板服务。

代理模式切换应放在以上步骤之后。USB 线不能替代 Universal Clipboard 使用的蓝牙发现和 Wi-Fi 近距离链路，因此插线通常不会修复共享剪贴板。
...
最终提示：可以尝试来回切换科学插件的“全局/规则”模式。如以上都无法修复，只有分别重启两端的设备，属于三十六计中的最后一计。
